      ...
      <Modal open={showModal} onClose={closeModal} title={editing ? `Editar ${itemLabel}` : `Novo ${itemLabel}`} onSubmit={handleSubmit} submitLabel="Salvar" width="540px">
        ...
        {form.categoriaInvestimento && isInvest && (() => {
          ...
          return (<>
            ...
            {(form.categoriaInvestimento === "ACOES" || form.categoriaInvestimento === "FIIS" || form.categoriaInvestimento === "ETFS") && (<><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}><NumberInput label="Preço Médio" required decimal value={form.valorUnitario} onChange={v => setForm({ ...form, valorUnitario: v })} error={errors.valorUnitario} placeholder="Ex: 35.50" /><NumberInput label="Quantidade" required value={form.quantidade} onChange={v => setForm({ ...form, quantidade: v })} error={errors.quantidade} placeholder="Ex: 100" /></div>{form.precoAtual ? (<div style={{ padding: "10px 14px", background: "#f8fafc", borderRadius: "10px", border: "1.5px solid #e2e8f0", display: "flex", justifyContent: "space-between", alignItems: "center" }}><span style={{ fontSize: "13px", fontWeight: 600, color: "#64748b" }}>📊 Preço atual (sistema)</span><span style={{ fontSize: "16px", fontWeight: 800, color: "#0f172a" }}>{fmt(parseFloat(form.precoAtual), moeda)}</span></div>) : (<div style={{ padding: "10px 14px", background: "#f8fafc", borderRadius: "10px", border: "1.5px solid #e2e8f0", fontSize: "13px", color: "#94a3b8" }}>Selecione um ativo para ver o preço atual</div>)}</>)}
            {form.categoriaInvestimento === "CRIPTOMOEDAS" && (<><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px" }}><NumberInput label="Preço Médio" required decimal value={form.valorUnitario} onChange={v => setForm({ ...form, valorUnitario: v })} error={errors.valorUnitario} placeholder="Ex: 350000" /><NumberInput label="Quantidade" required highPrecision value={form.quantidade} onChange={v => setForm({ ...form, quantidade: v })} error={errors.quantidade} placeholder="Ex: 0.05" /></div>{form.precoAtual ? (<div style={{ padding: "10px 14px", background: "#f8fafc", borderRadius: "10px", border: "1.5px solid #e2e8f0", display: "flex", justifyContent: "space-between", alignItems: "center" }}><span style={{ fontSize: "13px", fontWeight: 600, color: "#64748b" }}>📊 Preço atual (sistema)</span><span style={{ fontSize: "16px", fontWeight: 800, color: "#0f172a" }}>{fmt(parseFloat(form.precoAtual), moeda)}</span></div>) : (<div style={{ padding: "10px 14px", background: "#f8fafc", borderRadius: "10px", border: "1.5px solid #e2e8f0", fontSize: "13px", color: "#94a3b8" }}>Selecione um ativo para ver o preço atual</div>)}</>)}
          </>);
        })()}
